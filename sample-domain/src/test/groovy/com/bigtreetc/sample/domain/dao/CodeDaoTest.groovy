...
class CodeDaoTest extends BaseTestContainerSpec {
    ...
    def "存在しないIDで絞り込んだ場合、空のリストが返ること"() {
        when:
        def options = DomaUtils.createSelectOptions(Pageable.unpaged()).count()
    ...
    def "存在するデータを指定して更新した場合、更新件数が1件になること"() {
        when:
        def code = (codeDao.selectById(1) as Optional<Code>).orElseThrow {new NoDataFoundException("データが見つかりません。")}
